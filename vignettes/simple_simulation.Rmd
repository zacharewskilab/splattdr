...
sim.dose = c(0, 0.01, 0.03, 0.1, 0.3, 1, 3, 30)
dose.prob = rep(1/8, 8)
...
```{r}
de = rowData(sim)[which(rowData(sim)$DE_idx != 1), ]
de.genes = rownames(de)
```

```{r}
df = data.frame(dose = colData(simDR[[1]])$Dose, response = logcounts(simDR[[1]])[de.genes[1], ])
```
...
ggplot(data = df %>% filter(response != 0), aes(x = dose, y = response)) +
  geom_point() + 
  theme_bw()
```
